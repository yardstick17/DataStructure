...
void SumCheckCorrection(struct node *Root)
	   {  
	...
	if(Root == NULL || (Root->left==NULL && Root->right==NULL))
	...
	else
	 {
	   	  ...
	   	  SumCheckCorrection(Root->left);
		  SumCheckCorrection(Root->right);
	   	  ...
	   	  int diff= n1 + n2 - Root->data;
	   	  
		  if(diff > 0)
		   Root->data += diff;
		  
		  if(diff < 0)
	   	    Increment(Root, -diff);//SumCheckCorrection(Root->left);
       	  ...
       	  if(Root->left!=NULL)
       	    {
				 ...
				 Increment(Root->left,diff);//SumCheckCorrection(Root->left);
 
       	    }
       	    else
			{
			   	 Root->right->data = Root->right->data  + diff ;
		   	     Increment(Root->right,diff);//SumCheckCorrection(Root->right);
		    }
...
int main()
{
  ...
  SumCheckCorrection(Root);
  cout<<"\n Sum Check "<<SumCheck(Root);
    cout<<"\n"; 
  LevelOrder(Root); 
  cout<<"\n"; 
